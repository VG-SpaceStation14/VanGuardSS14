...
public sealed partial class LatheMenu : FancyWindow
{
    ...
    // VG-Tweak Start: avoids re-localising the label every frame.
    ...
    public void SetEntity(EntityUid uid)
    {
        ...
        // VG-Tweak Start: mining points claim UI
        if (_entityManager.TryGetComponent<MiningPointsComponent>(Entity, out var pointsComp))
        {
            MiningPointsContainer.Visible = true;
            MiningPointsClaimButton.OnPressed += _ => OnClaimMiningPoints?.Invoke();
            UpdateMiningPoints(pointsComp.Points);
        }
        else
        {
    ...
    // VG-Tweak Start
    /// <summary>
    /// Updates the mining points label and disables the claim button when there is nothing to claim.
    /// </summary>
    ...
    // VG-Tweak Start: keep the mining points label in sync while the window is open.
